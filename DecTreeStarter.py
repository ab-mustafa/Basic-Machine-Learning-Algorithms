...
from mlAlgo import DecTree_ID3
...
def dataShufflingDT(data):
    ...
    for i in range(10):
        ...
        for c in range(5):
            ...
            DT = DecTree_ID3.DecisionTree(data)
            DecisionTree = DT.ID3(trainSet, list(range(0, len(DataCopy[0])-1)))
            ...
            for record in range(len(testSet)):
                ...
                result = DT.extractFromTree(DecisionTree, input_rec)
...
if __name__ == '__main__':
    ...
    if dataset_Name == "letter":
        ...
        for index in columnIndexToChange:
            ...
            Discreate_array = ConvertRowToDiscreate(array, 30)
            SaveColumn(data, Discreate_array, index)

    if dataset_Name == "ecoli":
        # Remove 1st column
        data = extract.deleteColumn(data, 0)
        # for each column convert continuous to Discrete
        #data = extract.convertContToDisc(data, [0, 1, 2, 3, 4, 5, 6])
        # for each column convert continuous to Discrete :5,6
